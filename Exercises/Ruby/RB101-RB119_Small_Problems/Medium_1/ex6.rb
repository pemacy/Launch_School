...
# perform the operation using the popped value and the register value, and
...
def minilang(str)
  ...
  for command in str.split
    case command
    when 'PUSH' then stack << register.first
    when 'ADD'
      value = stack.pop + register.first
      register = [value]
    when 'SUB'
      value = register.first - stack.pop
      register = [value]
    when 'MULT'
      value = stack.pop * register.first
      register = [value]

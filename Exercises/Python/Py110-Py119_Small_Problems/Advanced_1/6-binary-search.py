...
def compare(list_term, search_term):
    if list_term == search_term:
        return 0
    elif search_term > list_term:
        return 1
    else:
        return -1
...
def binary_search(lst, search_term):
    ...
    if lst_len == 1:
        # print(f'Compare List Term: {lst[0]} with Search Term: {search_term} = {compare(lst[0], search_term)}')
        return 0 if compare(lst[0], search_term) == 0 else -1
    else:
        half_len = lst_len // 2

    match compare(lst[half_len], search_term):
        case 0:
            return half_len
        case -1:
            result = binary_search(lst[:half_len], search_term)
            return -1 if result == -1 else result
        case 1:
            result = binary_search(lst[half_len:], search_term)
            return -1 if result == -1 else half_len + result
